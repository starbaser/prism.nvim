...
T["stats"]["empty snapshot reports zeroes"] = function()
  ...
  eq(s.events, 0)
  eq(s.merged_events, 0)
  eq(s.capped_refreshes, 0)
  eq(s.burst_entries, 0)
  eq(s.burst_active, false)
end
...
T["stats"]["record_event tracks scheduler pressure"] = function()
  ...
  eq(active.events, 2)
  eq(active.merged_events, 1)
  eq(active.capped_refreshes, 1)
  eq(active.burst_entries, 1)
  eq(active.burst_active, true)

  stats.record_burst_exit()
  eq(stats.snapshot().burst_active, false)
end
